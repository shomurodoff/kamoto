...
import {updateTransferOwnership, getUser, deleteUser, deletePendingInvite} from '../core/_requests'
      ...
      <div className='card-body'>
        <div className='row d-flex'>
          <div className='col-md-6 col-12'>
            <h4 className='card-label fs-5'>{formatMessage({id: 'User & Roles'})}</h4>
            <p className='pt-3'>{formatMessage({id: 'Roles of registered Users'})}</p>
          </div>
                  ...
                  <tr className='custom-thead font-size-12 font-weight-400'>
                    ...
                    <td className='pb-4'>
                      <div className='d-flex gap-2'>
                        ...
                        <img
                          src={toAbsoluteUrl('/media/icons/profile/down_arrow.svg')}
                          alt=''
                          width={10}
                          className='mt-2'
                          height={7}
                    ...
                    user.roleId.includes('owner') ? (
                      <>
                        <tr>
                          ...
                          <td className=' min-w-175px'>{RolesName[user.roleId[0]]}</td>
                          <td className=' min-w-250px'>{user.email}</td>
                          <td className=' min-w-175px'>
                            {user.updatedAt
                              ? getDateValue(user.updatedAt?.toString())
                                ? getDateValue(user.updatedAt?.toString())
                                : DateTime.fromISO(user.updatedAt?.toString()).toLocaleString(
                                    DateTime.DATE_MED
                                  )
                              : '-'}{' '}
                            {user.updatedAt && getTimeZoneValue(user.updatedAt?.toString())}
                          </td>
                          <td
                        ...
                        <td>
                          {user.createdAt
                            ? getDateValue(user.createdAt?.toString())
                              ? getDateValue(user.createdAt?.toString())
                              : DateTime.fromISO(user.createdAt?.toString()).toLocaleString(
                                  DateTime.DATE_MED
                                )
                            : '-'}{' '}
                          {user.createdAt && getTimeZoneValue(user.createdAt?.toString())}
                        </td>
                        <td
