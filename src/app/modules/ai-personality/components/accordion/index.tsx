...
const Index: React.FC<any> = ({i, expanded, setExpanded, title, paragraph}) => {
  const isOpen = i === expanded
  return (
    <>
      <motion.header initial={false}>
        <h3 className={'text-[14px] leading-5 flex justify-between items-center'}>
          {title}
          <button
            className={'w-6 h-6 rounded-full flex items-center justify-center bg-[#2E2F45]'}
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <p className={'text-[12px] leading-5 text-[#FFFFFFA6] pt-[12px]'}>{paragraph}</p>
          </motion.section>
        )}
      </AnimatePresence>
    </>
