...
export function Registration() {
  ...
  const utmSource = new URL(window.location.href).searchParams.get('utm_source') || null
  const utmMedium = new URL(window.location.href).searchParams.get('utm_medium') || null
  const utmCampaign = new URL(window.location.href).searchParams.get('utm_campaign') || null
  ...
  if (!lsUtmSource) {
    localStorage.setItem('utm_source', JSON.stringify({firstTouch: utmSource}))
  } else {
  ...
  if (!lsUtmMedium) {
    localStorage.setItem('utm_medium', JSON.stringify({firstTouch: utmMedium}))
  } else {
    localStorage.setItem(
      'utm_medium',
  ...
  if (!lsUtmCampaign) {
    localStorage.setItem('utm_campaign', JSON.stringify({firstTouch: utmCampaign}))
  } else {
    localStorage.setItem(
      'utm_campaign',
  ...
  const handleAcceptInvite = async (values: any, {setSubmitting}: FormikProps) => {
    setLoading(true)
