...
const VerifyInvite = () => {
  ...
  useEffect(() => {
        ...
        if (success) {
          if (data.userExists) {
            ...
            setTimeout(() => {
              toast.success(
                formatMessage({
                  id: "Invite accepted. Please log into the correct account.",
                })
              );
            }, 500);
          } else {
            ...
            setTimeout(() => {
              toast.success(
                formatMessage({
                  id: "Please sign up below to accept the invite.",
                })
              );
            }, 500);
          }
    ...
    if (!userToken) {
      // Call verify invite API only if user is not logged in
      getVerifyToken();
    }
