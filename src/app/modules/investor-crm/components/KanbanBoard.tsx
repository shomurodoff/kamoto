...
export const KanbanBoard = ({
  ...
  const onDragEnd = (result: any, columns: any) => {
    ...
    if (source.droppableId !== destination.droppableId) {
      ...
      const sourceColumn =
        columns[findColumnIndex(parseInt(source.droppableId))].investors[investorIndex]

      //Adding investor from destination column
      if (columns[findColumnIndex(parseInt(destination.droppableId))].investors.length === 0) {
        columns[findColumnIndex(parseInt(destination.droppableId))].investors.push(sourceColumn)
      } else {
        columns[findColumnIndex(parseInt(destination.droppableId))].investors.splice(
          destination.index,
          0,
          sourceColumn
        )
      ...
      if (columns[findColumnIndex(parseInt(source.droppableId))].investors.length === 0) {
        columns[findColumnIndex(parseInt(source.droppableId))].investors = []
      } else {
        columns[findColumnIndex(parseInt(source.droppableId))].investors.splice(investorIndex, 1)
      }
      ...
      const destinationColumnData = columns.find(
        ({columnId}: {columnId: number}) => columnId === +destination.droppableId
      )
      if (sourceColumnData.status === '1' && destinationColumnData.status !== '1') {
      ...
      setUpdateColumnEntries({
        ...
        isFavourite:
          columns[findColumnIndex(parseInt(destination.droppableId))].investors[destination.index]
    ...
    function findColumnIndex(id: number) {
      return columns.findIndex(({columnId}: {columnId: number}) => columnId === id)
    }
  }
  ...
  useEffect(() => {
    const updateColumn = async () => {
      const findAffirmative = columns.find((column: {status: string}) => column.status === '1')
      const findNegative = columns.find((column: {status: string}) => column.status === '0')
  ...
  const onToggle = async (columnId: number, investorId: number, isFavourite: boolean) => {
  ...
  const getInvestorProgress = useCallback((investor: any, round: any): number | null => {
                      ...
                      index === columnIndex && (
                        <Dropdown className='dropdown-container'>
                          ...
                          <Dropdown.Menu>
                            ...
                            <Dropdown.Item onClick={() => handleDeleteColumn(column.columnId)}>
                    ...
                    <Droppable droppableId={column.columnId.toString()} key={column.columnId}>
                      {(provided: any, snapshot: any) => {
                        return (
                          ...
                          >
                            {column.investors?.map((investor: any, index: number) => {
                              return (
                                <Draggable
                                    ...
                                    return (
                                      <div
                                        ...
                                        style={{
                                          ...
                                          borderColor: snapshot.isDragging ? 'pink' : '',
                                          color: 'white',
                                            ...
                                            <div className='accordion-item'>
                                                    ...
                                                    <i
                                                      onClick={() => {
                                                        onToggle(
                                                          column.columnId,
                                                          investor.investorId,
                                                          !investor.investor_column.isFavourite
                                                        )
                                                      }}
                                              ...
                                              >
                                                <div>
                                                  ...
                                                  {getInvestorProgress(investor, column.round) !==
                                                    null && (
                                              ...
                                              <div className='d-flex justify-content-between ps-4 pb-3'>
                                                <small className='text-muted'>
                                                  {getDateValue(investor?.updatedAt?.toString())
                                                    ? getDateValue(investor?.updatedAt?.toString())
                                                    : DateTime.fromISO(
                              ...
                              {isHoveringForColumn && index === columnIndex ? (
                                column.status === '2' && (
                                  <button
                                    ...
                                    onClick={(e) => handleAddInvestorModal(true, column)}
                                  >
