...
import {addColumn, editColumn, getAllColumnsFromRoundId} from '../core/_requests'
...
export const CurrentRound = ({
  ...
  const onSubmit = useCallback(
    ...
    [columnDetails, color, selectedRoundId, setShow, setGetAllColumns, formatMessage]
  )
        ...
        <div className='w-100'>
          {showAll && (
            <div className='d-md-flex col-md-12 w-100'>
              <div className='d-flex flex-column justify-content-center col-md-3'>
                <p className='mb-0'> {formatMessage({id: 'Current Round'})}</p>
                <div>
              ...
              <div className='col-md-6 d-md-flex flex-md-column justify-content-center'>
                <div className='col-12 mb-2 fs-5'>
                  <span className='fw-bold '>
                    ...
                    {convertValueToShortScale(activeRound?.amountAchieved)} <span> </span> of {activeRound?.currency?.symbol}
                ...
                <div className='col-12 d-flex'>
                  <div className='col-6 h-6px progress  w-75'>
                    <div
                      ...
                      style={{
                        width: `${Math.ceil(
                          (activeRound?.amountAchieved / parseInt(activeRound?.amountTargeted)) *
                            100
                        )}%`,
                      }}
                    ></div>
                ...
                <div className='d-flex justify-content-start mt-2'>
                  ...
                  {activeRound?.allInvestors.length > 0 && investorImg.length > 4 && (
                    ...
                    investorImg.length !== 0 && (
                      <div className='align-items-lg-center d-flex mb-4 ps-3 pt-2'>
                        {activeRound?.allInvestors.length === investorImg.length
                          ? ``
                          : `+${activeRound?.allInvestors.length - investorImg.length} more`}
                      </div>
                    )}
                  {activeRound?.allInvestors.length > 0 && investorImg.length === 0 && (
                    <div className='align-items-lg-center d-flex mb-4 ps-3 pt-2'>
          ...
          {!showAll && (
            <div className='col-md-12 d-md-flex align-items-center'>
              <p className='mb-0 col-md-2'> {formatMessage({id: 'Current Round'})}</p>
              ...
              <div className='col-md-5 d-flex align-items-center '>
                <div className='col-md-10 h-6px progress  w-90'>
                  <div
                    ...
                    style={{
                      width: `${Math.ceil(
                        (activeRound?.amountAchieved / parseInt(activeRound?.amountTargeted)) * 100
                      )}%`,
                    }}
                  ></div>
                </div>
                ...
                >
                  ...
                  <Modal.Body>
                    <TextInput
                      ...
                      toolTipText={formatMessage({id: 'GLOBAL.TOOLTIP.INVESTOR.COLUMNAME'})}
                    />
                    <div>
                      <label className='form-label'>{formatMessage({id: 'Column Color'})}</label>
                      <ToolTipUI
                        tooltipText={formatMessage({id: 'GLOBAL.TOOLTIP.INVESTOR.COLUMNCOLOR'})}
                      />
                        ...
                        {colorArray &&
                          colorArray.map((item) => (
                            <button
                              ...
                              onClick={() => {
                                setColor(item)
                                setColumnsDetails({...columnDetails, color: item})
                              }}
                            >
