...
import {deleteInvestorColumn, getRoundById, updateColumnFromApi} from '../core/_requests'
...
export const InvestorDetails = ({
  ...
  useEffect(() => {
    if (investor) {
      const find = investor?.columns.find((column: any) => column.columnId === state?.columnId)
      if (find) {
  ...
  const handleChange = async (event: any) => {
    ...
    const status =
      state.columns[
        state.columns.findIndex((column: any) => column.columnId === +event.target.value)
      ].status
    ...
    <>
      ...
      <div className='d-sm-flex justify-content-between p-2'>
        <div>
          ...
          <p className='align-items-center d-flex font-size-12 text-primary'>
            ...
            <Link to='/investor-crm'>{formatMessage({id: 'Investor CRM'})}</Link>
            <span className='h-1px bullet mx-2'></span>
            <span className='text-muted font-size-12'> {formatMessage({id: 'Investor'})}</span>
          </p>
        </div>
          ...
          <div className='d-flex justify-content-between'>
            ...
            <Dropdown className='dropdown-container '>
              ...
              <Dropdown.Menu className='m-2'>
                ...
                >
                  <p className='mb-0'>{formatMessage({id: 'Remove Investor'})}</p>
                </Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>
          </div>
      ...
      <div className='d-flex p-4 mt-5 mt-md-10 amount-container'>
        <div className='d-flex col justify-content-between'>
          <div className={`col-8 d-flex align-items-center ${!transaction && 'd-none'}`}>
            <div className='h-8px w-100 rounded' style={{backgroundColor: '#a1a5b7'}}>
              <div
                className='rounded h-8px'
                role='progressbar'
                style={{
                  width: `${Math.min(
                    Math.ceil((transaction / parseInt(round?.amountTargeted)) * 100),
                    100
                  )}%`,
            ...
            <span className='ps-2'>
              <small>{Math.ceil((transaction / parseInt(round?.amountTargeted)) * 100)}%</small>
            </span>
          </div>
          <div className='col-2'>
            ...
            {transaction ? (
              <div className='d-flex gap-1'>
                <p className='fw-bold'>{investor?.currency?.symbol ? investor?.currency?.symbol : "$"}</p>
                <p className='fw-bold'>{convertValueToShortScale(transaction)}</p>
              </div>
            ) : (
              'NA'
            )}
