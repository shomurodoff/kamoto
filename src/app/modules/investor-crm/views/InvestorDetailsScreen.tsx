...
export const InvestorDetailsScreen = () => {
  ...
  const [sort_by_investor, set_sort_by_investor] = useState<string | undefined>(undefined)
  const [sort_order_investor, set_sort_order_investor] = useState<string | undefined>(undefined)
  ...
  useEffect(() => {
    if (allRounds && activeRound) {
      setSelectedRoundId(
        allRounds[allRounds?.findIndex((round: any) => round?.roundId === activeRound?.roundId)]
          ?.roundId
      )
    }
                ...
                <div className='align-self-center col-lg-5 col-md-5 col-xl-5 col-12 d-flex ps-0'>
                  <div className='w-50'>
                    <select
                      ...
                      onChange={(e) => handleOnChangeRound(parseInt(e.target.value))}
                    ...
                    <Dropdown className='dropdown-container'>
                      ...
                      <Dropdown.Menu className='m-2'>
                        <Dropdown.Item className='text-dark'>
                          <p className='mb-0'>{formatMessage({id: 'Import CSV'})}</p>
                        </Dropdown.Item>

                        <Dropdown.Item className='text-dark' onClick={onExportInvestors}>
                          <p className='mb-0'>{formatMessage({id: 'Export to csv'})}</p>
                        </Dropdown.Item>
                        <Dropdown.Item className='text-dark' onClick={() => setAddModal(!addModal)}>
                          <p className='mb-0'>{formatMessage({id: 'Rearrange Columns'})}</p>
                        </Dropdown.Item>
                        <Dropdown.Item className='text-dark' onClick={addRoundcolumn}>
                          <p className='mb-0'>{formatMessage({id: 'Add new Column'})}</p>
                        </Dropdown.Item>
                        <Dropdown.Item onClick={() => setDeleteShowModal(!showDeleteModal)}>
                                ...
                                <Droppable droppableId='columns'>
                                  {(provided) => (
                                    <div ref={provided.innerRef} {...provided.droppableProps}>
                                      {getAllColumns.map((column: any, index: number) => (
                                        <Draggable
                                          key={column.columnId}
                                        ...
                                        >
                                          {(provided) => (
                                            ...
                                            >
                                              <div>
                                                <img
                                                  src={'/media/icons/investor/hamburger.svg'}
                                                  alt=''
                                                />
                                              </div>
                                              <div>
