...
import {InvestmentRejectionInitialValues, rejectionReasons} from '../core/_constants'
import {getInvestorResponseAPI, rejectTransactionApi} from '../core/_requests'
...
export const InvestmentRejectionmModal = ({
  ...
  const intializeInvestmentRejectionSchema = Yup.object().shape({
    rejectReason: Yup.string().required(formatMessage({id: 'Rejection reason is required'})),
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      const {
        data: {success, errors},
      } = await rejectTransactionApi({...values, investorUserId: +values.investorUserId})
      if (success) {
        setLoading(false)
        if (getInvestor) {
          getInvestor()
    ...
    <>
      ...
      >
        <Modal.Header closeButton>
          <div className='fs-4 fw-bold'>{formatMessage({id: 'Investment Rejection Details'})}</div>
        </Modal.Header>
        <Formik
                ...
                <Modal.Body>
                  <div className='row d-md-flex justify-content-between'>
                    <div className='col-md-6'>
                      <SelectInput
                        ...
                        placeholder={formatMessage({id: 'Select Cancellation reason'})}
                        formik={formik}
                    ...
                    <div className='col-md-6'>
                      <SelectInput
                        label={formatMessage({id: 'Contact person who rejected'})}
                        fieldName={'investorUserId'}
                        placeholder={formatMessage({id: 'Select the contact name who said no'})}
                        formik={formik}
                        toolTipText={formatMessage({
                          id: 'GLOBAL.TOOLTIP.INVESTOR.REJECTION_REASON',
                        })}
                  ...
                  <div className='row d-md-flex justify-content-between'>
                    <div className='col-md-6'>
                      <TextInput
                        ...
                        toolTipText={formatMessage({id: 'GLOBAL.TOOLTIP.INVESTOR.REJECTION_DATE'})}
                        isStarRequired={true}
                      />
                    </div>
                    <div className='col-md-6'>
                  ...
                  <div>
                    <TextArea
                      ...
                      label={formatMessage({id: 'Feedback or detailed reason'})}
                      toolTipText={formatMessage({
